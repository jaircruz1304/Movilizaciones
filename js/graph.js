import { acquireToken } from './auth.js?v=2.0.0';
import { sleep } from './utils.js?v=2.0.0';
...
export async function graph(path,options={}){
  ...
  const {authScopes,...fetchOptions}=options||{};
  let attempt=0;
  while(attempt<3){
    const token=await acquireToken(authScopes);
...
export async function graphPaged(path,max=20000,authScopes=null){
  const rows=[]; let url=path;
  while(url && rows.length<max){
    const data=await graph(url,authScopes?{authScopes}:{}); rows.push(...(data?.value||[])); url=data?.['@odata.nextLink']||'';
  }
